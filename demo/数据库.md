...
CREATE TABLE `user_follow_news` (
  ...
  `news_uniquekey` VARCHAR(50) NOT NULL COMMENT '新闻唯一标识（替代原news_id）',
  `news_title` VARCHAR(200) NOT NULL COMMENT '新闻标题（便于本地查看）',
  `news_time` DATETIME NULL COMMENT '新闻发布时间',
  `news_author` VARCHAR(200) NULL COMMENT '新闻发布者',
  `follow_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '关注时间',
  PRIMARY KEY (`follow_id`),
  -- 外键约束：关联用户表，用户删除时同步删除其关注记录
  FOREIGN KEY (`user_id`) REFERENCES `user` (`user_id`) ON DELETE CASCADE,
  -- 联合唯一约束：避免用户重复关注同一篇新闻（基于user_id和news_uniquekey）
  UNIQUE KEY `uk_user_news_uniquekey` (`user_id`, `news_uniquekey`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用户关注新闻表：存储用户与新闻的关注关系';



以下是上个版本的user_follow_news表，目前已经废除：
以下是上个版本的user_follow_news表，目前已经废除：
以下是上个版本的user_follow_news表，目前已经废除：
